// AnchorMapCtrl.h: interface for the CAnchorMapCtrl class.
//
//////////////////////////////////////////////////////////////////////

#if !defined(AFX_ANCHORMAPCTRL_H__5BE4BE70_5F51_424A_9015_2D2E1E9A5E09__INCLUDED_)
#define AFX_ANCHORMAPCTRL_H__5BE4BE70_5F51_424A_9015_2D2E1E9A5E09__INCLUDED_
...
//					ANCHOR_MAP_ENTRY(IDOK,         AMF_BOTTOM)
//					ANCHOR_MAP_ENTRY(IDCANCEL,     AMF_BOTTOM)
//					ANCHOR_MAP_ENTRY(IDC_EDITCTRL, AMF_TOP | RIGHT | AMF_BOTTOM)
//					ANCHOR_MAP_ENTRY(NULL,         AMF_AUTOMATIC)
//				END_ANCHOR_MAP()
//
...
// Note: You should not combine docking(AMF_DOCK_...) and anchoring flags.
//       You can try it, but it�s not tested.
//       Additionally, AMF_AUTOMATIC should not be used with any other flags
#define AMF_NONE			0x0000                  /* proportinally moves the control with the size of the window */
#define AMF_DOCK_TOP		0x0001                  /* docks the control to the top of the window */
#define AMF_DOCK_BOTTOM		0x0002                  /* docks the control to the bottom of the window */
#define AMF_DOCK_LEFT		0x0004                  /* docks the control to the left of the window */
#define AMF_DOCK_RIGHT		0x0008                  /* docks the control to the right of the window */
#define AMF_DOCK_ALL		0x000F                  /* docks the control to all sides of the window */    
#define AMF_TOP				0x0010                  /* distance of the control to the top of the window will be constant */
#define AMF_BOTTOM			0x0020                  /* distance of the control to the bottom of the window will be constant */
#define AMF_LEFT			0x0040                  /* distance of the control to the left of the window will be constant */
#define AMF_RIGHT			0x0080                  /* distance of the control to the right of the window will be constant */
#define AMF_AUTOMATIC		0x0100                  /* automatically calculate the anchors, cannot be used with other flags */
#define AMF_DOCK_TOP_EX		0x0200                  /* docks the top of the control to the top of the window */
#define AMF_DOCK_BOTTOM_EX	0x0400                  /* docks the bottom of the control to the bottom of the window */
#define AMF_DOCK_LEFT_EX	0x0800                  /* docks the left-side of the control to the left-side of the window */
#define AMF_DOCK_RIGHT_EX	0x1000                  /* docks the right-side of the control to the right-side of the window */
// some additional control flags
#define AMF_ERASE			0x2000                  /* forces to erase the background of the control in EraseBackground */
// some combinations
#define AMF_TOPLEFT			(AMF_TOP | AMF_LEFT)    
#define AMF_TOPRIGHT		(AMF_TOP | AMF_RIGHT)
#define AMF_BOTTOMLEFT		(AMF_BOTTOM | AMF_LEFT)
#define AMF_BOTTOMRIGHT		(AMF_BOTTOM | AMF_RIGHT)
#define AMF_TOPBOTTOM		(AMF_TOP | AMF_BOTTOM)
#define AMF_LEFTRIGHT		(AMF_LEFT | AMF_RIGHT)
// flags for InitAnchors                    
#define AMIF_CALCSIZE		0x0001                  /* calculate size occupied by all controls, useful for formviews */
#define AMIF_SIZEGRIP		0x0002                  /* add a sizing-grip to the parent window */

static const int MAX_CONTROLS = 256;

class CAnchorMapCtrl
{
public:
...
public:
	...
	BOOL ScreenToClient(HWND hWnd, RECT* pRect)
	{
		...
		pt2.x = pRect->right;
		pt2.y = pRect->bottom;
		if(FALSE == ::ScreenToClient(hWnd, &pt1))
			return FALSE;
		if(FALSE == ::ScreenToClient(hWnd, &pt2))
			return FALSE;
		pRect->left = pt1.x;
		pRect->top = pt1.y;
		pRect->right = pt2.x;
	...
	void PreProcess(RECT* pWndRect)
	{
		...
		// (we can use our defined AMF_ constants here for simplicity)
		m_uiSizedBorders = 0;
		if(m_rcNew.left != m_rcPrev.left)        m_uiSizedBorders |= AMF_LEFT;
		if(m_rcNew.top != m_rcPrev.top)          m_uiSizedBorders |= AMF_TOP;
		if(m_rcNew.right != m_rcPrev.right)      m_uiSizedBorders |= AMF_RIGHT;
		if(m_rcNew.bottom != m_rcPrev.bottom)    m_uiSizedBorders |= AMF_BOTTOM;
	...
	static BOOL CALLBACK InitDefaultControl(HWND hWnd, LPARAM lParam)
	{
		...
		CAnchorMapCtrl* pMap = (CAnchorMapCtrl*)(lParam);

		// do some validation
		if(NULL == hWnd || NULL == pMap)
			return FALSE;
	...
	// ======================================================================
	void Initialize(HWND hWndParent, DWORD dwFlags/* = 0*/, int nOffsetRight/* = 0*/)
	{
		int		iCtrl = 0;
		HWND	hWndCtrl = NULL;
		RECT	rcCtrl;
		...
		if(dwFlags & AMIF_SIZEGRIP)
		{
			dw1				= m_nDefaultFlags;
			m_nDefaultFlags	= AMF_RIGHT | AMF_BOTTOM;
		...
		if(dwFlags & AMIF_CALCSIZE)
		{
			for(iCtrl = 0;iCtrl < m_nCtrlCount;++iCtrl)
			{
				if(m_Ctrl[iCtrl].rect.right > rcMaxBR.right)
					rcMaxBR.right = (long)m_Ctrl[iCtrl].rect.right + m_nOffsetRight;
				if(m_Ctrl[iCtrl].rect.bottom > rcMaxBR.bottom)
					rcMaxBR.bottom = (long)m_Ctrl[iCtrl].rect.bottom;
			}
		...
		for(iCtrl = 0;iCtrl < m_nCtrlCount;++iCtrl)
		{
			...
			if(m_Ctrl[iCtrl].nFlags == AMF_AUTOMATIC)
			{
				...
				if(m_Ctrl[iCtrl].rect.top < (szClient.cy / 2))
					m_Ctrl[iCtrl].nFlags |= AMF_TOP;
				if(m_Ctrl[iCtrl].rect.bottom >= (szClient.cy / 2))
					m_Ctrl[iCtrl].nFlags |= AMF_BOTTOM;
				...
				if(m_Ctrl[iCtrl].rect.left < (szClient.cx / 2))
					m_Ctrl[iCtrl].nFlags |= AMF_LEFT;
				if(m_Ctrl[iCtrl].rect.right >= (szClient.cx / 2))
					m_Ctrl[iCtrl].nFlags |= AMF_RIGHT;
			}
		}
	...
	// Note that docking-flags (AMF_DOCK_...) have a higher privilege
	// than the normal anchoring flags. If an AMF_DOCk_... flag is specified
	...
	void HandleAnchors(RECT* pRectParent)
	{
		...
		for(iCtrl = 0;iCtrl < m_nCtrlCount;++iCtrl)
		{
			...
			// handle anchoring
			if(m_uiSizedBorders & AMF_LEFTRIGHT && m_szDelta.cx != 0 && ! bChanged)
			{
				switch(pCtrl->nFlags & AMF_LEFTRIGHT)
				{
				case AMF_LEFT: // nothing to do here, control moves automatically
					// with the left-border of the window (client-rect)
					break;
				case AMF_RIGHT:
					pCtrl->rect.left += m_szDelta.cx;
					pCtrl->rect.right = pCtrl->rect.left + szCtrl.cx;
					bChanged = TRUE;
					break;
				case AMF_LEFTRIGHT:
					pCtrl->rect.right += m_szDelta.cx;
					bChanged = TRUE;
					break;
				default:
					pCtrl->rect.left += (double)m_szDelta.cx / 2.0;
					pCtrl->rect.right = pCtrl->rect.left + szCtrl.cx;
					bChanged = TRUE;
					break;
				}
			...
			if(m_uiSizedBorders & AMF_TOPBOTTOM && m_szDelta.cy != 0)
			{
				switch(pCtrl->nFlags & AMF_TOPBOTTOM)
				{
				case AMF_TOP:// nothing to do here, control moves automatically
					// with the top of the window (client-rect);
					break;
				case AMF_BOTTOM:
					pCtrl->rect.top += m_szDelta.cy;
					pCtrl->rect.bottom = pCtrl->rect.top + szCtrl.cy;
					bChanged = TRUE;
					break;
				case AMF_TOPBOTTOM:
					pCtrl->rect.bottom += m_szDelta.cy;
					bChanged = TRUE;
					break;
				default:
					pCtrl->rect.top += (double)m_szDelta.cy / 2.0;
					pCtrl->rect.bottom = pCtrl->rect.top + szCtrl.cy;
					bChanged = TRUE;
					break;
				}
	...
	BOOL EraseBackground(HDC hDC)
	{
		...
		while(hWndChild != NULL)
		{
			...
			if(-1 != iCtrl)
			{
				if((m_Ctrl[iCtrl].nFlags & AMF_ERASE) == 0)
					iCtrl = -1;
			}
...
//
// DECLARE_ANCHOR_MAP declares the variable m_AnchorMap within your window
...
#define DECLARE_ANCHOR_MAP() CAnchorMapCtrl m_AnchorMap; \
	void InitAnchors(DWORD dwFlags = 0, int nOffsetRight = 0); \
	void HandleAnchors(RECT *pRect);

#define BEGIN_ANCHOR_MAP(theclass) void theclass::HandleAnchors(RECT *pRect){ \
	m_AnchorMap.HandleAnchors(pRect); \
}; \
	void theclass::InitAnchors(DWORD dwFlags, int nOffsetRight) {

#define ANCHOR_MAP_ENTRY(nIDCtrl, nFlags) m_AnchorMap.AddControl(nIDCtrl, nFlags);

#define ANCHOR_MAP_ENTRY_RANGE(nIDCtrlFrom, nIDCtrlTo, nFlags) \
	for(int iCtrl = nIDCtrlFrom;iCtrl <= nIDCtrlTo;++iCtrl) \
	m_AnchorMap.AddControl(iCtrl, nFlags);

#define END_ANCHOR_MAP() m_AnchorMap.Initialize(m_hWnd, dwFlags, nOffsetRight); \
	RECT rcWnd;  \
	::GetWindowRect(m_hWnd, &rcWnd); \
	m_AnchorMap.HandleAnchors(&rcWnd); \
};

#endif // !defined(AFX_ANCHORMAPCTRL_H__5BE4BE70_5F51_424A_9015_2D2E1E9A5E09__INCLUDED_)
